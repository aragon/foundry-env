...
# $3: Solidity script file name
# $4: Explorer API URL (optional for "sourcify")
# $5: Explorer API Key (optional)
...
usage() {
  echo "Usage:"
  echo "  $(basename "$0") <chain_id> <explorer_type> <script_file_name> <explorer_api_url> [explorer_api_key]"
  echo
  echo "Example (Etherscan or compatible):"
  echo "  $(basename "$0") 11155111 etherscan MyScript https://api-sepolia.etherscan.io/api YOUR_ETHERSCAN_KEY"
  echo "  $(basename "$0") 43113 custom MyScript https://api.routescan.io/v2/network/testnet/evm/43113/etherscan ''"
  echo
  echo "Example (Blockscout):"
  echo "  $(basename "$0") 100 blockscout MyScript https://blockscout.com/xdai/mainnet/api YOUR_BLOCKSCOUT_KEY"
  echo
  echo "Example (Sourcify):"
  echo "  $(basename "$0") 11155111 sourcify MyScript \"\" \"\""
  echo
...
locate_source_file() {
  ...
  (find src | grep -i "/$contract_name.sol\$") || \
...
RUN_LATEST_JSON_PATH="broadcast/${SCRIPT_FILENAME}/${CHAIN_ID}/run-latest.json"

if [[ ! -f "$RUN_LATEST_JSON_PATH" ]]; then
  echo "Error: Broadcast file not found at ${RUN_LATEST_JSON_PATH}"
  echo "Ensure you have run 'forge script ${SCRIPT_FILENAME} --chain-id ${CHAIN_ID} --broadcast ...' first."
  exit 1
fi
